...
# \d 表示匹配一个数字， \D 表示非数字
mystring = "_xiao@l356780ing#$%^&ggg^^^25wangleli"
print("****aaa....")
print("****aaa....")
...
# \w 匹配大小写字母数字加上_ python 变量命名 ,  \W就是匹配非大小写字母数字和_
print(re.search(r"\w\w\w", mystring))
print(re.search(r"\W", mystring))

# *前面字符任意次
print(re.search(r"\d*", mystring))#应该是0次或多次
print(re.search(r"\D*", mystring))
print(re.search(r"\d+", mystring))#极可能多的匹配 贪婪
print(re.search(r"\d+?", mystring))#非贪婪
...
#.表示匹配一个字符 +？非贪婪
#表示匹配''符号里所有的值也就是#member_id#而不是member_id
pattern = r'#.+?#'
print(re.search(pattern, login_data))

# result = re.sub(pattern, "1987", login_data, count=1)
# print(result)
#
